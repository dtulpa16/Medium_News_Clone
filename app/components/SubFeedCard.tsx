...
const SubFeedCard: React.FC<SubFeedCardProps> = ({ article }) => {
  return (
    <div className="flex xl:h-[175px] md:h-150 xl:gap-6 md:gap-4">
      <div className="flex flex-col text-black gap-2 max-w-[400px]">
        <Link href="" className="text-sm">
          {article.sourceName}
        </Link>
        <h3 className="line-clamp-2 font-bold text-2xl ">{article.title}</h3>
        <h3 className="line-clamp-2 text-lg ">{article.body || bodyPlaceholder}</h3>
