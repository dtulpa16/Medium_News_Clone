...
const PlanCard = ({ title, price, features, highlighted } : Plan) => {
    return (
      <div className={`p-6 bg-white rounded-lg shadow-md ${highlighted ? 'border-2 border-yellow-400' : ''}`}>
        {highlighted && <div className="text-yellow-400 text-2xl absolute top-4 right-4">★</div>}
        <h3 className="text-2xl font-semibold mb-2">{title}</h3>
        <p className="text-lg font-medium text-gray-700 mb-4">{price}</p>
        ...
        <ul>
          {features.map((feature, index) => (
            <li key={index} className="flex items-center text-sm text-gray-700 mb-1">
